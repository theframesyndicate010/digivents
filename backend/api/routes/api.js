...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        if (req.path.includes('/projects')) dest += 'projects/';
        else if (req.path.includes('/creators')) dest += 'creators/';
        else if (req.path.includes('/graphics')) dest += 'graphics/';
...
router.delete('/graphics/:id', async (req, res) => {
    try {
        ...
        const id = parseInt(req.params.id, 10);
        if (Number.isNaN(id)) {
            return res.status(400).json({ success: false, message: 'Invalid graphic id' });
        }
