document.addEventListener('DOMContentLoaded', function () {
	...
	function socialLinks(client) {
		const links = [];
		if (client.instagram_link) links.push('<a href="' + ui.escapeHtml(client.instagram_link) + '" target="_blank" class="text-pink-600"><i class="fab fa-instagram"></i></a>');
		if (client.facebook_link) links.push('<a href="' + ui.escapeHtml(client.facebook_link) + '" target="_blank" class="text-blue-600"><i class="fab fa-facebook"></i></a>');
		if (client.tiktok_link) links.push('<a href="' + ui.escapeHtml(client.tiktok_link) + '" target="_blank" class="text-black"><i class="fab fa-tiktok"></i></a>');
		if (!links.length) links.push('<span class="text-sm text-gray-400">-</span>');
		return links.join(' ');
	}
	...
	function renderClients(clients) {
		...
		if (!items.length) {
			const emptyRow = '<tr><td colspan="5" class="text-center py-8 text-gray-500"><i class="fas fa-briefcase text-4xl mb-2 block"></i>No clients found.</td></tr>';
