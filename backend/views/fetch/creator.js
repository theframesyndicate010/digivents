		...
		if (tableBody) {
			tableBody.innerHTML = items.map(function (creator) {
				...
				return '<tr>' +
					...
					'<td class="px-6 py-4 whitespace-nowrap text-sm font-medium"><a href="/admin/edit-creator/' + id + '" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a><button class="text-red-600 hover:text-red-900" onclick="deleteCreator(\'' + id + '\')">Delete</button></td>' +
				'</tr>';
			}).join('');
		}
		...
		if (mobileList) {
			mobileList.innerHTML = items.map(function (creator) {
				...
				return '<div class="bg-white shadow rounded-lg p-4">' +
					...
					'<div class="flex justify-end space-x-3 border-t pt-3"><a href="/admin/edit-creator/' + id + '" class="text-indigo-600 font-medium text-sm">Edit</a><button class="text-red-600 font-medium text-sm" onclick="deleteCreator(\'' + id + '\')">Delete</button></div>' +
				'</div>';
			}).join('');
		}
	}
