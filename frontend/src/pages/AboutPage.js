            ...
            {team.map((member, i) => (
              ...
              >
                <div className="relative overflow-hidden rounded-2xl mb-5 border border-white/[0.06] group-hover:border-white/15 transition-colors duration-300">
                  <img
                    src={member.image || 'https://images.unsplash.com/photo-1511367461989-f85a21fda167?w=400&h=600&fit=crop'}
                    alt={member.name}
                    className="w-full h-72 object-cover transition-transform duration-500 ease-out group-hover:scale-105"
                    loading="lazy"
                    onError={(e) => {
                      console.error('Image failed to load:', member.image);
                      e.target.src = 'https://images.unsplash.com/photo-1511367461989-f85a21fda167?w=400&h=600&fit=crop';
                    }}
                  />
